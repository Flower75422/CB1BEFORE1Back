...
export default function PoolFormone({ data, update }: any) {
  // 🔴 Pulls the exact same pool array from the parent state
  ...
  return (
    <div className="space-y-2 mt-2">
      ...
      {/* 🔴 NEW: 'group' class added for linked hover effects */}
      <div className="relative group">
        <Tag 
          size={14} 
          className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors duration-200 ${currentPool.length === 0 ? 'text-stone-300' : 'text-stone-400 group-hover:text-[#1c1917]'}`} 
        />
        ...
        <select 
          ...
          // 🔴 NEW: Added hover:bg-stone-50, hover:border-stone-400, and strict disabled states
        ...
        >
          {currentPool.length === 0 ? (
            ...
            // 🔴 NEW: Dynamically maps options from the selected tags
        ...
        {/* Adds an alert icon if the user needs to select tags first */}
        {currentPool.length === 0 && (
      ...
      <p className="text-[9px] font-semibold text-stone-400 ml-1">
        {currentPool.length === 0 
          ? "You must select at least one interest above to set your primary category."
          : "This defines the main category your group belongs to."
        }
      </p>
    </div>
  );
