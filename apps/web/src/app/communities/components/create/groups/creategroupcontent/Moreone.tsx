...
export default function Moreone({ data, update, setIsSuccess }: any) {
  ...
  const { myGroups, updateGroup } = useCommunitiesStore();
  ...
  const handleLaunch = () => {
    if (!data.title?.trim() || !data.groupId?.trim()) {
      showErr("Group Name & ID required!");
      return;
    }

    const nameConflict = myGroups.some(
      (g) => g.id !== data.id && g.name.toLowerCase() === data.title.trim().toLowerCase()
    );
    if (nameConflict) {
      showErr("Group name already taken!");
      return;
    }

    const idConflict = myGroups.some(
      (g) => g.id !== data.id &&
        g.handle.split("/")[0].replace("@", "").toLowerCase() === data.groupId.trim().toLowerCase()
    );
    if (idConflict) {
      showErr("Group ID already taken!");
      return;
    }
