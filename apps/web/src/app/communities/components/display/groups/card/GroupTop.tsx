...
import { ShieldCheck, MoreVertical, Share, LogOut, UserPlus, EyeOff, Settings, Check, BellOff, Clock, Info, CalendarDays, MapPin, History, ChevronDown, Copy } from "lucide-react";
...
export default function GroupTop({
  id, title, members, isPrivate, avatarUrl, isJoined, isOwner, isMuted, isPending, handle,
  onManage, onJoin, onRequest, onCancelRequest, onLeave, onHide, onAbout,
}: GroupTopProps) {
    ...
    <div className="flex justify-between items-start relative z-20">
      ...
      <div className="flex items-center gap-0 shrink-0">
        ...
        {!isOwner && !isJoined && !isPending && (
          <button
            onClick={(e) => { e.stopPropagation(); isPrivate ? onRequest?.() : onJoin?.(); }}
            ...
            <div className="absolute right-0 top-8 w-48 bg-white rounded-xl shadow-xl border border-stone-200/60 z-50 py-1.5 animate-in fade-in zoom-in-95 duration-100 origin-top-right">
              ...
              {isOwner ? (
                <button onClick={(e) => { e.stopPropagation(); onManage?.(); setIsMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-[11px] font-bold text-[#1c1917] hover:bg-stone-50 transition-colors">
                  <Settings size={14} className="text-stone-400" /> Manage Group
                </button>
              ) : (
                <>
                  ...
                  {isJoined ? (
                    <button onClick={(e) => { e.stopPropagation(); onLeave?.(); setIsMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-[11px] font-bold text-[#1c1917] hover:bg-stone-50 transition-colors">
                      <LogOut size={14} className="text-stone-400" /> Leave Group
                    </button>
                  ) : (
                    <button onClick={(e) => { e.stopPropagation(); onJoin?.(); setIsMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-[11px] font-bold text-[#1c1917] hover:bg-stone-50 transition-colors">
