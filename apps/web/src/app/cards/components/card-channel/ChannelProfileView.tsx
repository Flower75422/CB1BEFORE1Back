...
export default function ChannelProfileView({ data, onCloseInfo }: any) {
  ...
  const { mutedChannelIds, muteChannel, unmuteChannel, unsubscribeChannel } = useCommunitiesStore();
  const { closeAllViews } = useCardsSearchStore();

  const channelId: string = data?.channelHandle ?? data?.handle?.replace("@", "") ?? "";
  const isMuted = mutedChannelIds?.includes(channelId) ?? false;
          ...
          <div className="animate-in fade-in slide-in-from-left-4 duration-200">
            ...
            <div className="p-6 pb-20 flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-1">
                <button
                onClick={() => channelId && (isMuted ? unmuteChannel(channelId) : muteChannel(channelId))}
              ...
              <div className="w-full p-3.5 rounded-xl bg-[#F5F5F4] border border-transparent flex justify-between items-center">
                <div className="flex items-center gap-3">{isPublic ? <Globe size={16} className="text-stone-500" /> : <Lock size={16} className="text-stone-500" />}<span className="text-[13px] font-bold text-[#1c1917]">{isPublic ? "Public Channel" : "Private Channel"}</span></div>
                <div className={`w-9 h-5 rounded-full relative ${isPublic ? 'bg-blue-500' : 'bg-stone-300'}`}><div className={`absolute top-1 w-3 h-3 bg-white rounded-full shadow-sm ${isPublic ? 'translate-x-5' : 'left-1'}`}></div></div>
              </div>
              </div>
