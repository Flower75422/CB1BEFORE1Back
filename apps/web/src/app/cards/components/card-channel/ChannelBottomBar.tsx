...
export default function ChannelBottomBar({ channelId, isOwner, isPrivate }: ChannelBottomBarProps) {
  const {
  ...
  return (
    <div className="h-[72px] bg-white border-t border-stone-100 px-4 flex items-center justify-center gap-3 shrink-0">
      {isSubscribed ? (
        <button
          onClick={() => id && unsubscribeChannel(id)}
        ...
        <>
          ...
          <button className="flex items-center gap-1.5 px-4 py-2 bg-[#1c1917] text-white rounded-full text-[11px] font-black uppercase tracking-widest hover:bg-black active:scale-95 transition-all">
            <UserPlus size={13} /> Request to Subscribe
          </button>
        </>
      ) : (
        <button
          onClick={() => id && subscribeChannel(id)}
